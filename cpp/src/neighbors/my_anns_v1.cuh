...
void index<T, IdxT>::precompute_entry_point_norms(raft::resources const& res)
{
  ...
  if (!query_norms_.has_value()) { allocate_entry_point_norms(res); }
  if (metric() == cuvs::distance::DistanceType::CosineExpanded) {
    raft::linalg::rowNorm(query_norms_->data_handle(),
 ...
 *
 * The kNN graph is the first building block for my_anns_v1 index.
 *
 ...
 *
 * See [my_anns_v1::build](#my_anns_v1::build) for an alternative method.
 *
 ...
 *   // create knn graph
 *   my_anns_v1::build_knn_graph(res, dataset, knn_graph.view(), 2, build_params, search_params);
 *   auto optimized_gaph = raft::make_host_matrix<IdxT, IdxT>(dataset.extent(0), 64);
 *   my_anns_v1::optimize(res, dataset, knn_graph.view(), optimized_graph.view());
 *   // Construct an index from dataset and optimized knn_graph
 *   auto index = my_anns_v1::index<T, IdxT>(res, build_params.metric(), dataset,
...
void build_knn_graph(
  ...
  my_anns_v1::graph_build_params::ivf_pq_params ivf_pq_params)
{
 ...
 *   auto knn_graph      = raft::make_host_matrix<IdxT, IdxT>(dataset.extent(0), 128);
 *   // create knn graph
 *   my_anns_v1::build_knn_graph(res, dataset, knn_graph.view(), build_params);
 *   auto optimized_gaph      = raft::make_host_matrix<IdxT, int64_t>(dataset.extent(0), 64);
 *   my_anns_v1::optimize(res, dataset, nn_descent_index.graph.view(), optimized_graph.view());
 *   // Construct an index from dataset and optimized knn_graph
 *   auto index = my_anns_v1::index<T, IdxT>(res, build_params.metric(), dataset,
 * optimized_graph.view());
 * @endcode
 ...
 * @code{.cpp}
 *   using namespace cuvs::neighbors;
 *   my_anns_v1::index_params build_params;
 *   auto knn_graph = raft::make_host_matrix<IdxT, IdxT>(dataset.extent(0), 128);
 *   // build KNN graph not using `my_anns_v1::build_knn_graph`
  ...
  raft::mdspan<IdxT, raft::matrix_extent<int64_t>, raft::row_major, g_accessor> knn_graph)
{
  ...
  my_anns_v1::detail::graph::sort_knn_graph(res, dataset_internal, knn_graph_internal);
}
 ...
 *
 * See [my_anns_v1::build_knn_graph](#my_anns_v1::build_knn_graph) for usage example
 *
...
{
  return my_anns_v1::detail::build<T, IdxT, Accessor>(res, params, dataset);
}
 ...
 *   my_anns_v1::search_with_filtering(res, search_params, index, queries, neighbors, distances,
 *     filtering::bitset_filter(removed_indices_bitset.view()));
 * @endcode
 *
 * @tparam T data element type
 * @tparam IdxT type of the indices
 * @tparam my_anns_v1SampleFilterT Device filter function, with the signature
 ...
 */
template <typename T, typename IdxT, typename my_anns_v1SampleFilterT>
                           ...
                           my_anns_v1SampleFilterT sample_filter = my_anns_v1SampleFilterT())
{
  ...
  return my_anns_v1::detail::search_main<T, internal_IdxT, my_anns_v1SampleFilterT, IdxT>(
...
{
  my_anns_v1::extend_core<T, IdxT, Accessor>(handle, additional_dataset, index, params, ndv, ngv);
}
...
{
  return my_anns_v1::detail::merge<T, IdxT>(handle, params, indices);
}

/** @} */  // end group my_anns_v1
