  ...
  for (int offset = WARP_SIZE / 2; offset > 0; offset /= 2) {
    ...
    if (_cand >= 0) {
      if (cand == -1) {
        ...
        bool update = reverse ? lt(pq[cand].distance, pq[_cand].distance)
                                              ...
                                              const NeighborIdxT nodeid)
{
  ...
  for (int i = threadIdx.x; i < size; i += blockDim.x) {
    if (pq[i].nodeid == nodeid) { exists = true; }
  }
  ...
  if (warp == 0) {
    ...
    if (threadIdx.x == 0) { shared[0] = exists; }
  }
  __syncthreads();
  return shared[0];
}
                                  ...
                                  const bool reverse)
{
  ...
  if (warp == 0) {
    cand = warp_reduce_cand(pq, cand, reverse);
    // broadcast back to shared memory
    if (threadIdx.x == 0) { shared[0] = cand; }
  }
  __syncthreads();
  return shared[0];
}
...
__inline__ __device__ bool CheckVisited(int* visited_table,
                                        int* visited_list,
                                        int& visited_cnt,
                                        int target,
