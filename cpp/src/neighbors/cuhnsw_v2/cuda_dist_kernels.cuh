...
__inline__ __device__ cuda_scalar warp_reduce_sum(cuda_scalar val)
{
...
__inline__ __device__ cuda_scalar dot(const cuda_scalar* a,
                                      const cuda_scalar* b,
                                      const int num_dims)
{
  __syncthreads();
...
__inline__ __device__ cuda_scalar squaresum(const cuda_scalar* a,
                                            const cuda_scalar* b,
                                            const int num_dims)
{
  __syncthreads();
...
__inline__ __device__ cuda_scalar GetDistanceByVec(const cuda_scalar* src_vec,
                                                   const cuda_scalar* dst_vec,
                                                   const int num_dims,
                                                   const int dist_type)
{
  cuda_scalar dist = 0;
  switch (dist_type) {
    case DOT: dist = -dot(src_vec, dst_vec, num_dims); break;
    case L2: dist = squaresum(src_vec, dst_vec, num_dims); break;
    default: break;
  }
  return dist;
}
...
__inline__ __device__ cuda_scalar GetDistance2(
  const int src, const int dst, const int num_dims, const int dist_type, const cuda_scalar* data)
{
...
__global__ void BatchDistanceKernel(const cuda_scalar* data,
                                    const int* src,
                                    const int* dst,
                                    const int size,
                                    const int num_dims,
