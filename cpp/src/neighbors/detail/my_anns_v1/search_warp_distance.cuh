...
struct search : public search_plan_impl<DataT, IndexT, DistanceT, SAMPLE_FILTER_T> {
  ...
  lightweight_uvector<INDEX_T> candidate_indices_buffer;
  lightweight_uvector<DISTANCE_T> candidate_distances_buffer;
  ...
  void set_params(raft::resources const& res, const search_params& params)
  {
    ...
    RAFT_EXPECTS(max_queries == 1, "max_queries must be 1 when 'search_mode' is \"warp-distance\"");
  }
