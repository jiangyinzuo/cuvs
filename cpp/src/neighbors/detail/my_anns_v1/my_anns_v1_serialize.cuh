 ...
 * @param[in] index_ my_anns_v1 index
 *
 */
...
{
  raft::common::nvtx::range<cuvs::common::nvtx::domain::cuvs> fun_scope("my_anns_v1::serialize");

  RAFT_LOG_DEBUG(
    "Saving my_anns_v1 index, size %zu, dim %u", static_cast<size_t>(index_.size()), index_.dim());
  ...
  if (include_dataset) {
    RAFT_LOG_INFO("Saving my_anns_v1 index with dataset");
    neighbors::detail::serialize(res, os, index_.data());
  } else {
    RAFT_LOG_DEBUG("Saving my_anns_v1 index WITHOUT dataset");
  }
}
...
void serialize_to_hnswlib(
  ...
  std::ostream& os,
  const my_anns_v1::index<T, IdxT>& index_,
  std::optional<raft::host_matrix_view<const T, int64_t, raft::row_major>> dataset)
{
  ...
  raft::common::nvtx::range<cuvs::common::nvtx::domain::cuvs> fun_scope("my_anns_v1::serialize");
  RAFT_LOG_DEBUG("Saving my_anns_v1 index to hnswlib format, size %zu, dim %u",
  ...
  if (dataset) {
    ...
    RAFT_EXPECTS(dataset.size() > 0,
                 "Invalid my_anns_v1 dataset of size 0 during serialization, shape %zux%zu",
...
{
  raft::common::nvtx::range<cuvs::common::nvtx::domain::cuvs> fun_scope("my_anns_v1::deserialize");
