  ...
  unsigned MULTI_WARPS = 0)
{
  ...
  if (MULTI_WARPS == 0) {
    ...
    for (unsigned i = 0; i < N; i++) {
      ...
      // NOTE(jiangyinzuo): only consider the first num_itopk candidates
...
{
  DEBUG_PRINTF("MULTI_WARPS_1, MULTI_WARPS_2: %d, %d, num_candidates: %u, num_itopk: %u\n",
               MULTI_WARPS_1,
               MULTI_WARPS_2,
               num_candidates,
               num_itopk);
...
#ifndef NDEBUG
  const auto result_buffer_size    = num_itopk + num_candidates;
  const auto result_buffer_size_32 = raft::round_up_safe<uint32_t>(result_buffer_size, 32);
  print_result_buffer(
    result_buffer_size_32, itopk_indices, itopk_distances, __FILE__, __LINE__, 9999);
#endif
...
)
{
  ...
  static_assert(std::is_same_v<EntryPointsPolicy, ComputeRandomEntryPointsOneWarp<INDEX_T>> ||
                "Unsupported EntryPointsPolicy");
  DEBUG_PRINTF("MAX_ITOPK: %d, MAX_CANDIDATES: %d, TOPK_BY_BITONIC_SORT: %d\n",
               MAX_ITOPK,
               MAX_CANDIDATES,
...
#define _CLK_REC(V)
  ...
  } else if constexpr (std::is_same_v<EntryPointsPolicy,
                                      ComputeRandomEntryPointsOneWarp<INDEX_T>>) {
    entry_points_policy(
      warp_result_index, warp_result_distance, dataset_desc, graph_degree, visited_table);
  }
  ...
  while (1) {
...
#ifndef NDEBUG
    if (warp_id == 8 && threadIdx.x % warp_size() == 0) {
      printf("xx warp_id: %u, graph_degree: %u, warp_result_index: %u\n",
             warp_id,
             graph_degree,
             warp_result_index);
    }
    ...
    if (blockIdx.x == PRINT_BLOCK_IDX_X && threadIdx.x == 0 && std::is_same_v<float, DISTANCE_T> &&
      ...
      printf("iter %u, candidate_indices_buffer:", iter);
      for (unsigned i = 0; i < graph_degree; i++) {
        if (i % 16 == 0) { printf("\n"); }
        printf("%10u ", candidate_indices_buffer[i]);
      }
      printf("\n");
    }
    __syncthreads();
    ...
    for (unsigned i = threadIdx.x; i < graph_degree; i += blockDim.x) {
      result_distances_buffer[i + internal_topk] = candidate_distances_buffer[i];
      result_indices_buffer[i + internal_topk]   = candidate_indices_buffer[i];
    }
    __syncthreads();
    print_result_buffer(result_buffer_size_32,
                        result_indices_buffer,
    ...
    //       topk_by_bitonic_sort<MAX_ITOPK, INDEX_T>(
    ...
    //           _CLK_START();
    //           hashmap::init(visited_table.table, visited_table.bitlen, 32);
    //           _CLK_REC(clk_reset_hash);
    // #ifdef _GRAPH_QUALITY_ANALYSIS
    //           if (METRIC_THREAD_COND()) { atomicAdd(&my_anns_v1_metrics->counter_reset_hash,
    //           1UL); }
    // #endif
    //         }
    //       }
    ...
    // _CLK_START();
    const auto smem_parent_id = parent_list_buffer[0];
    if (smem_parent_id != invalid_index) {
      const auto parent_id = result_indices_buffer[smem_parent_id] & ~index_msb_1_mask;
      device::compute_distance_to_one_child_node_one_warp(warp_result_index,
  ...
  if (blockIdx.x == 0) {
    ...
    print_result_buffer(result_buffer_size_32,
                        ...
                        __LINE__,
                        iter);
    print_result_buffer(top_k, result_indices_ptr, result_distances_ptr, __FILE__, __LINE__, iter);
...
)
{
  ...
  search_core<MAX_ITOPK,
              ...
              EntryPointsPolicy>(result_indices_ptr,
                                 ...
                                 knn_graph,
                                 graph_degree,
                                 candidate_distances_buffer,  // [graph_degree]
                                 candidate_indices_buffer,    // [graph_degree]
                                                              // num_entry_points,
                                 internal_topk,
                                 search_width,
                                 min_iteration,
                    ...
                    cudaStream_t stream)
{
  if (ps.persistent) {
    ...
    RAFT_EXPECTS(ps.search_width == 1, "search width must be 1");
    dim3 thread_dims(block_size, 1, 1);
    dim3 block_dims(graph.extent(1) / (block_size / 32), num_queries, 1);
#ifndef NDEBUG
    RAFT_LOG_INFO("Launching kernel with %u threads, %ux%u blocks %u smem",
                  block_size,
                  graph.extent(1) / (block_size / 32),
                  num_queries,
                  smem_size);
#endif
    ...
    void* args[]      = {reinterpret_cast<void*>(&topk_indices_ptr),
                         reinterpret_cast<void*>(&topk_distances_ptr),
                         reinterpret_cast<void*>(&topk),
                         (void*)(&dataset_ptr),
                         reinterpret_cast<void*>(const_cast<DataT**>(&queries_ptr)),
                         (void*)(&graph_ptr),
                         reinterpret_cast<void*>(&graph_degree),
                         reinterpret_cast<void*>(&candidate_distances_buffer),
                         reinterpret_cast<void*>(&candidate_indices_buffer),
                         reinterpret_cast<void*>(const_cast<size_t*>(&ps.itopk_size)),
                         reinterpret_cast<void*>(const_cast<size_t*>(&ps.search_width)),
                         reinterpret_cast<void*>(const_cast<size_t*>(&ps.min_iterations)),
                         reinterpret_cast<void*>(const_cast<size_t*>(&ps.max_iterations)),
                         reinterpret_cast<void*>(&num_executed_iterations),
                         reinterpret_cast<void*>(&sample_filter),
                         reinterpret_cast<void*>(const_cast<EntryPointsPolicy*>(&entry_points_policy)),
                         reinterpret_cast<void*>(&visited_table)
#ifdef _GRAPH_QUALITY_ANALYSIS
                      ,
                    reinterpret_cast<void*>(&my_anns_v1_metrics)
#endif
    };
    cudaLaunchCooperativeKernel(
      reinterpret_cast<void*>(kernel), block_dims, thread_dims, args, smem_size, stream);
