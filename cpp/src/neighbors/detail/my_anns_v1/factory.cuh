...
template <typename DataT,
          typename IndexT,
          typename DistanceT,
          typename my_anns_v1SampleFilterT = cuvs::neighbors::filtering::none_sample_filter>
class factory {
 public:
  /**
                  ...
                  const dataset_descriptor_host<DataT, IndexT, DistanceT>& dataset_desc)
  {
    if (plan.algo == search_algo::SINGLE_CTA) {
      return std::make_unique<
        single_cta_search::search<DataT, IndexT, DistanceT, my_anns_v1SampleFilterT>>(
      ...
      return std::make_unique<
        multi_kernel_search::search<DataT, IndexT, DistanceT, my_anns_v1SampleFilterT>>(
        res, plan, dataset_desc, plan.dim, plan.dataset_size, plan.graph_degree, plan.topk);
    }
  }
};
...
auto make_key(const my_anns_v1::search_params& params,
 ...
 * `neighbors/detail/my_anns_v1/compute_distance-ext.cuh`).
 *
