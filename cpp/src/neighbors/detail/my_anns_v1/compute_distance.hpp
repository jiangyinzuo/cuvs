...
using init_desc_type = dataset_descriptor_host<DataT, IndexT, DistanceT> (*)(
  const my_anns_v1::search_params&, const DatasetT&, cuvs::distance::DistanceType);
...
template <typename InstanceSpec,
          ...
          typename DistanceT,
          typename DatasetT>
constexpr auto spec_match(const my_anns_v1::search_params& params,
 ...
 * `neighbors/detail/my_anns_v1/compute_distance-ext.cuh`, which is generated by
 * `neighbors/detail/my_anns_v1/compute_distance_00_generate.py`.
...
struct instance_selector {
  template <typename DataT, typename IndexT, typename DistanceT, typename DatasetT>
  static auto select(const my_anns_v1::search_params&, const DatasetT&, cuvs::distance::DistanceType)
    -> std::tuple<init_desc_type<DataT, IndexT, DistanceT, DatasetT>, double>
  {
    return std::make_tuple(nullptr, -1.0);
  }
...
struct instance_selector<Spec, Specs...> {
  template <typename DataT, typename IndexT, typename DistanceT, typename DatasetT>
  static auto select(const my_anns_v1::search_params& params,
                     const DatasetT& dataset,
                     cuvs::distance::DistanceType metric)
