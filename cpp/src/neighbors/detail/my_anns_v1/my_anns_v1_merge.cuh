...
{
  using my_anns_v1_index_t = my_anns_v1::index<T, IdxT>;
  using ds_idx_type   = typename my_anns_v1_index_t::dataset_index_type;

  std::size_t dim              = 0;
  std::size_t new_dataset_size = 0;
  int64_t stride               = -1;

  for (my_anns_v1_index_t* index : indices) {
    ...
    if (auto* strided_dset = dynamic_cast<const strided_dataset<T, ds_idx_type>*>(&index->data());
      ...
      new_dataset_size += index->size();
               ...
               nullptr) {
      RAFT_FAIL(
        "my_anns_v1::merge only supports an index to which the dataset is attached. Please check if the "
        "index was built with index_param.attach_dataset_on_build = true, or if a dataset was "
        "attached after the build.");
    } else {
      RAFT_FAIL("my_anns_v1::merge only supports an uncompressed dataset index");
    }
  }
  ...
  my_anns_v1::index_params output_index_params = params.output_index_params;
  ...
  try {
    ...
    auto merged_index =
      my_anns_v1::build(handle, output_index_params, raft::make_const_mdspan(updated_dataset.view()));
    ...
    RAFT_LOG_DEBUG("my_anns_v1 merge: using device memory for merged dataset");
    return merged_index;

  } catch (std::bad_alloc& e) {
    RAFT_LOG_DEBUG("my_anns_v1::merge: using host memory for merged dataset");
