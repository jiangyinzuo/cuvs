...
// #include "topk_for_my_anns_v1/topk.h"
      ...
      topk(topk)
  {
    if (algo == search_algo::AUTO) {
      ...
      if (itopk_size <= 512 && search_params::max_queries >= num_sm * 2lu) {
        ...
        algo = search_algo::SINGLE_NEIGHBOR_LIST_MULTI_CTA_V2;
...
struct search_plan_impl : public search_plan_impl_base {
  ...
  lightweight_uvector<MyAnnsV1Metrics> my_anns_v1_metrics;
      ...
      dataset_desc(dataset_desc)
  {
    ...
    my_anns_v1_metrics.resize(1, raft::resource::get_cuda_stream(res));
  ...
  void adjust_search_params()
  {
    ...
    if (max_iterations == 0) {
      if (algo == search_algo::MULTI_CTA ||
          algo == search_algo::SINGLE_NEIGHBOR_LIST_MULTI_CTA_V1) {
        constexpr uint32_t mc_itopk_size   = 32;
        constexpr uint32_t mc_search_width = 1;
        _max_iterations                    = mc_itopk_size / mc_search_width;
      } else {
        _max_iterations = max(itopk_size / search_width - 1, (uint64_t)0);
      }
      int64_t num_reachable_nodes = 1;
      while (num_reachable_nodes + (int64_t)num_entry_points < dataset_size) {
  ...
  inline void calc_hashmap_params(raft::resources const& res)
  {
    ...
    if (algo == search_algo::MULTI_CTA || algo == search_algo::SINGLE_NEIGHBOR_LIST_MULTI_CTA_V1) {
  ...
  inline void check_params()
  {
    ...
    } else if (algo == search_algo::SINGLE_NEIGHBOR_LIST_MULTI_CTA_V1 ||
               algo == search_algo::SINGLE_NEIGHBOR_LIST_MULTI_CTA_V2) {
      if (team_size != 32) {
    ...
    if (error_message.length() != 0) { THROW("[my_anns_v1 Error] %s", error_message.c_str()); }
  }
};
