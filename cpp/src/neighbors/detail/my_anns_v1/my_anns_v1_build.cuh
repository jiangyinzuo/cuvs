...
void build_knn_graph(
  ...
  raft::mdspan<const DataT, raft::matrix_extent<int64_t>, raft::row_major, accessor> dataset,
  raft::host_matrix_view<IdxT, int64_t, raft::row_major> knn_graph,
  my_anns_v1::graph_build_params::ivf_pq_params pq)
{
  ...
  raft::common::nvtx::range<cuvs::common::nvtx::domain::cuvs> fun_scope(
    "my_anns_v1::build_graph(%zu, %zu, %u)",
  ...
  cuvs::neighbors::nn_descent::index_params build_params)
{
  ...
  my_anns_v1::detail::graph::sort_knn_graph(res, dataset, knn_graph_internal);
}
  ...
  const bool guarantee_connectivity = false)
{
  ...
  my_anns_v1::detail::graph::optimize(
  ...
  raft::mdspan<const T, raft::matrix_extent<int64_t>, raft::row_major, Accessor> dataset)
{
  ...
  try {
    ...
    RAFT_LOG_ERROR("Iterative my_anns_v1 graph build requires the dataset to fit GPU memory");
    throw e;
  }
  ...
  my_anns_v1_graph = raft::make_host_matrix<IdxT, int64_t>(initial_graph_size, small_graph_degree);
  for (uint64_t i = 0; i < initial_graph_size; i++) {
    for (uint64_t j = 0; j < small_graph_degree; j++) {
      my_anns_v1_graph(i, j) = (i + offset(j)) % initial_graph_size;
    }
  }
  ...
  while (true) {
    ...
    search_params.algo        = my_anns_v1::search_algo::AUTO;
    ...
    auto idx = index<T, IdxT>(
      res, params.metric, dev_dataset_view, raft::make_const_mdspan(my_anns_v1_graph.view()));
  ...
  raft::mdspan<const T, raft::matrix_extent<int64_t>, raft::row_major, Accessor> dataset)
{
  ...
  if (std::holds_alternative<my_anns_v1::graph_build_params::iterative_search_params>(
        knn_build_params)) {
    my_anns_v1_graph = iterative_build_graph<T, IdxT, Accessor>(res, params, dataset);
  } else {
    ...
    if (std::holds_alternative<my_anns_v1::graph_build_params::ivf_pq_params>(knn_build_params)) {
      auto ivf_pq_params =
        std::get<my_anns_v1::graph_build_params::ivf_pq_params>(knn_build_params);
      build_knn_graph(res, dataset, knn_graph->view(), ivf_pq_params);
    } else {
      auto nn_descent_params =
        std::get<my_anns_v1::graph_build_params::nn_descent_params>(knn_build_params);
      ...
      // Use nn-descent to build my_anns_v1 knn graph
      nn_descent_params.return_distances = false;
      build_knn_graph<T, IdxT>(res, dataset, knn_graph->view(), nn_descent_params);
    }

    my_anns_v1_graph = raft::make_host_matrix<IdxT, int64_t>(dataset.extent(0), graph_degree);

    RAFT_LOG_INFO("optimizing graph");
    optimize<IdxT>(res, knn_graph->view(), my_anns_v1_graph.view(), params.guarantee_connectivity);
  ...
  if (params.attach_dataset_on_build) {
    try {
      ...
      index_with_dataset.precompute_dataset_norms(res);
