...
// #include <raft/neighbors/my_anns_v1_types.hpp>
...
void add_node_core(
  ...
  const my_anns_v1::index<T, IdxT>& idx,
  ...
  const my_anns_v1::extend_params& extend_params)
{
  ...
  my_anns_v1::search_params params;
  ...
  for (const auto& batch : additional_dataset_batch) {
    // Step 1: Obtain K (=base_degree) nearest neighbors of the new vectors by my_anns_v1 search
    // Create queries
...
void add_graph_nodes(
  ...
  const neighbors::my_anns_v1::index<T, IdxT>& index,
  ...
  const my_anns_v1::extend_params& params)
{
  ...
  neighbors::my_anns_v1::index<T, IdxT> internal_index(
    handle,
    index.metric(),
  ...
  for (std::size_t additional_dataset_offset = 0; additional_dataset_offset < num_new_nodes;
    ...
    neighbors::my_anns_v1::add_node_core<T, IdxT>(
  ...
  std::optional<raft::device_matrix_view<IdxT, int64_t>> new_graph_buffer_view)
{
  ...
  if (auto* strided_dset = dynamic_cast<const strided_dataset<T, ds_idx_type>*>(&index.data());
    ...
    // Add graph nodes
    my_anns_v1::add_graph_nodes<T, IdxT>(
    ...
    if (new_graph_buffer_view.has_value()) {
      ...
      index.update_graph(handle, raft::make_const_mdspan(updated_graph.view()));
             ...
             nullptr) {
    RAFT_FAIL(
      "my_anns_v1::extend only supports an index to which the dataset is attached. Please check if the "
      "index was built with index_param.attach_dataset_on_build = true, or if a dataset was "
      "attached after the build.");
  } else {
    RAFT_FAIL("my_anns_v1::extend only supports an uncompressed dataset index");
  }
}
}  // namespace cuvs::neighbors::my_anns_v1
