...
#include "search_single_neighbor_list_multi_cta_v2_inst.cuh"
...
extern template void select_and_run<{data_t}, {idx_t}, {distance_t}, {sample_filter}, {entry_points_policy}, {visited_table}>(
    ...
    {distance_t}* candidate_distances_buffer,
    {idx_t}* candidate_indices_buffer,
...
for type_path, (data_t, idx_t, distance_t) in search_types.items():
    for entry_points_policy, entry_points_path in (
            (f"ComputeRandomEntryPointsOneWarp<{idx_t}>", "compute_random"),
            ):
        for visited_table, visited_table_path in (
            ...
            path = f"search_single_neighbor_list_multi_cta_v2_{type_path}_{entry_points_path}_{visited_table_path}.cu"
...
with open("search_single_neighbor_list_multi_cta_v2_kernel-ext.cuh", "w") as f:
    includes = '''
#pragma once
...
void select_and_run(const dataset_descriptor_host<DataT, IndexT, DistanceT>& dataset_desc,
    ...
    for content in extern_template_contents:
        f.write(content)
    f.write(trailer)
    print("src/neighbors/detail/my_anns_v1/search_single_neighbor_list_multi_cta_v2_kernel-ext.cuh")
