...
template <cuvs::distance::DistanceType Metric,
          ...
          typename IndexT,
          typename DistanceT>
struct my_anns_v1_q_dataset_descriptor_t : public dataset_descriptor_base_t<DataT, IndexT, DistanceT> {
  ...
  _RAFT_HOST_DEVICE my_anns_v1_q_dataset_descriptor_t(setup_workspace_type* setup_workspace_impl,
  ...
  {
    my_anns_v1_q_dataset_descriptor_t::encoded_dataset_ptr(args) = encoded_dataset_ptr;
    my_anns_v1_q_dataset_descriptor_t::vq_code_book_ptr(args)    = vq_code_book_ptr;
    this->pq_code_book_ptr()                                = pq_code_book_ptr;
    my_anns_v1_q_dataset_descriptor_t::encoded_dataset_dim(args) = encoded_dataset_dim;
    static_assert(sizeof(*this) == sizeof(base_type));
    static_assert(alignof(my_anns_v1_q_dataset_descriptor_t) == alignof(base_type));
  }
 ...
 private:
  RAFT_INLINE_FUNCTION constexpr static auto get_smem_ws_size_in_bytes(uint32_t dim) -> uint32_t
  {
    ...
    */
    return sizeof(my_anns_v1_q_dataset_descriptor_t) + kSMemCodeBookSizeInBytes +
...
RAFT_KERNEL __launch_bounds__(1, 1)
  vpq_dataset_descriptor_init_kernel(dataset_descriptor_base_t<DataT, IndexT, DistanceT>* out,
                                     ...
                                     uint32_t dim)
{
  using desc_type = my_anns_v1_q_dataset_descriptor_t<Metric,
                                                 TeamSize,
                                                 DatasetBlockDim,
                                                 PqBits,
                                                 PqLen,
...
vpq_descriptor_spec<Metric,
                    ...
                    DataT,
                    IndexT,
                    DistanceT>::init_(const my_anns_v1::search_params& params,
                                      const std::uint8_t* encoded_dataset_ptr,
                                      uint32_t encoded_dataset_dim,
                                      const CodebookT* vq_code_book_ptr,
                                      const CodebookT* pq_code_book_ptr,
