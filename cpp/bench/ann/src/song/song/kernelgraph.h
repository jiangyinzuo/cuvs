...
// TODO(jiangyinzuo): try astar_accelerator
//
...
// 	virtual void search_top_k_batch(const std::vector<std::vector<std::pair<int,value_t>>>& queries,int k,std::vector<std::vector<idx_t>>& results, size_t finish_cnt = 1){};
...
private:
    ...
    dist_t distance(idx_t a,T& b){
		if(metric_type == SongMetricType::L2)
        	return data->l2_distance(a,b);
		else if(metric_type == SongMetricType::IP)
    ...
    void qsort(size_t l,size_t r){
        ...
        if((size_t)i < r)qsort(i,r);
        if(l < (size_t)j)qsort(l,j);
    }
    ...
    void add_edge(idx_t v_id,idx_t u_id){
        auto offset = v_id << vertex_offset_shift;
        if(edges[offset] < (idx_t)flexible_degree){
            ++edges[offset];
            edges[offset + edges[offset]] = u_id;
            if(edges[offset] == (idx_t)flexible_degree){
                rank_edges(offset);
...
public:
    ...
    void add_vertex(idx_t vertex_id,std::vector<std::pair<int,value_t>>& point){
        ...
        idx_t num_neighbors = (idx_t)degree < (idx_t)neighbor.size() ? (idx_t)degree : (idx_t)neighbor.size();
        ...
        for(size_t i = 0;i < neighbor.size() && i < (size_t)degree;++i){
    ...
    void astar_multi_start_search(const std::vector<std::pair<int,value_t>>& query,int k,std::vector<idx_t>& result){
        ...
        for(int i = 0;i < num_start_point && i < (int)data->curr_vertices();++i){
        ...
        for(int iter = 0;iter < max_step && !q.empty();++iter){
            auto now = q.top();
			if(topk.size() == (size_t)k && topk.top().first < now.first){
                break;
            }
            ++explore_cnt;
    ...
    void print_stat(){
        ...
        for(size_t i = 0;i < n;++i){
            ...
            // FIXME(jiangyinzuo): may core dump
