...
__global__
void independent_search_kernel(value_t* d_data,value_t* d_query,idx_t* d_result,idx_t* d_graph,int num_query,int vertex_offset_shift, int TOPK){
    ...
    //VanillaList bf;
    KernelPair<dist_t,idx_t> *q = new KernelPair<dist_t, idx_t>[QUEUE_SIZE + 2];
    ...
    KernelPair<dist_t,idx_t> *topk = new KernelPair<dist_t, idx_t>[TOPK + 1];
...
public:
    static void astar_multi_start_search_batch(const std::vector<std::vector<std::pair<int,value_t>>>& queries,int k,std::vector<std::vector<idx_t>>& results,value_t* h_data,idx_t* h_graph,int vertex_offset_shift,int num,int dim){
		...
		std::unique_ptr<idx_t[]> h_result = std::unique_ptr<idx_t[]>(new idx_t[queries.size() * k]);

		cudaMalloc(&d_data,sizeof(value_t) * num * dim);
		cudaMalloc(&d_query,sizeof(value_t) * queries.size() * dim);
		cudaMalloc(&d_result,sizeof(idx_t) * queries.size() * k);
		...
		independent_search_kernel<<<600,128>>>(d_data,d_query,d_result,d_graph,queries.size(),vertex_offset_shift, k);
		cudaMemcpy(h_result.get(),d_result,sizeof(idx_t) * queries.size() * k,cudaMemcpyDeviceToHost);
		results.clear();
		for(size_t i = 0;i < queries.size();++i){
			std::vector<idx_t> v(k);
			for(int j = 0;j < k;++j)
				v[j] = h_result[i * k + j];
			results.push_back(v);
		}
