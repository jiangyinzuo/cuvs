...
class HierarchicalNavigableSmallWorld : public GraphWrapper {
 private:
  ...
  void DistributePoints(int num_of_initial_neighbors)
  {
    ...
    for (int i = 1; i < num_of_layers_; i++) {
      num_of_points_on_each_layer_[i] =
        (num_of_points_on_each_layer_[i] + num_of_points_one_batch_ - 1) /
        num_of_points_one_batch_ * num_of_points_one_batch_;
    }
  }
  ...
  void UpdateEdges(int last_point_id, int previous_point_id, float distance)
  {
    ...
    if (position_of_insertion != -1) {
      for (int i = num_of_final_neighbors - 2; i >= position_of_insertion; i--) {
        first_subgraph[position_of_neighbors_of_previous_point + i + 1] =
          first_subgraph[position_of_neighbors_of_previous_point + i];
      }

      first_subgraph[position_of_neighbors_of_previous_point + position_of_insertion] =
        std::make_pair(distance, last_point_id);
    }
  }
 ...
 public:
  ...
  void AddPointinGraph(int point_id, float* point)
  {
    ...
    for (int i = 0; i < (int)neighbors.size() && i < num_of_initial_neighbors_; i++) {
  ...
  void SearchTopK(float* query_point, int k, std::vector<std::pair<float, int>>& result)
  {
    ...
    std::priority_queue<pair<float, int>> topk;
    ...
    for (int iteration_id = 0; iteration_id < max_step && !pq.empty(); iteration_id++) {
      ...
      if ((int)topk.size() == k && topk.top().first < now_candidate.first) { break; }
      ...
      for (int i = 0; i < num_of_initial_neighbors_; i++) {
        ...
        if (neighbor_id >= points_->GetNumPoints()) { break; }

        if (visited.count(neighbor_id)) { continue; }

        pq.push(std::make_pair(distance(points_->GetFirstPositionofPoint(neighbor_id), query_point),
                               neighbor_id));
  ...
  void SearchTopKonDevice(
    float* queries, int num_of_topk, int*& results, int num_of_query_points, int num_of_candidates)
  {
    int num_of_topk_ = pow(2.0, ceil(log(num_of_topk) / log(2)));
    auto err = cudaMallocHost(&results, sizeof(int) * num_of_query_points * num_of_topk_);
    if (err != cudaSuccess) {
      cout << "Error in allocating memory for results. num_of_query_points=" << num_of_query_points << ", num_of_topk_=" << num_of_topk_ 
    ...
    HNSWGraphOperations<metric_type, DIM>::Search(points_->GetFirstPositionofPoint(0),
                                ...
                                num_of_initial_neighbors_,
                                num_of_final_neighbors,
                                num_of_topk_,
                                num_of_candidates,
                                num_of_explored_points,
                                num_of_layers_,
                                num_of_points_on_each_layer_);
    // std::chrono::steady_clock::time_point t2 = std::chrono::steady_clock::now();
    // std::cout << "Query speed: "
    //           << (double)num_of_query_points /
    //                ((double)std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count() /
    //                 1000000)
  ...
  void Establishment(int num_of_initial_neighbors, int num_of_candidates)
  {
    ...
    auto err = cudaMallocHost(&graph_,
                   sizeof(int) * (points_->GetNumPoints() * num_of_final_neighbors +
                                  num_of_points_on_nonbottom_layers * num_of_final_neighbors));
    if (err != cudaSuccess) {
      cout << "Error in allocating memory for graph. num_of_points=" << points_->GetNumPoints() << ", num_of_final_neighbors=" << num_of_final_neighbors 
        << "err: " << cudaGetErrorString(err) << endl;
      exit(1);
    }
    ...
    vector<pair<float, int>> substitute(num_of_points_one_batch_ * num_of_final_neighbors,
                                        neighbor_intialisation);
  ...
  void Dump(string graph_name)
  {
    ...
    out_descriptor.write((char*)graph_,
                         sizeof(int) *
                           (num_of_points_on_each_layer_[0] + num_of_points_on_nonbottom_layers) *
                           num_of_final_neighbors);
  ...
  void Load(string graph_path)
  {
    ...
    cudaMallocHost(&graph_,
                   sizeof(int) *
                     (num_of_points_on_each_layer_[0] + num_of_points_on_nonbottom_layers) *
                     num_of_final_neighbors);
    in_descriptor.read((char*)graph_,
                       sizeof(int) *
                         (num_of_points_on_each_layer_[0] + num_of_points_on_nonbottom_layers) *
                         num_of_final_neighbors);
    in_descriptor.close();
