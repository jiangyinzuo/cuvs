...
class Data {
 private:
  ...
  void ReadVectorsFromFiles(string path)
  {
    ...
    if (!in_descriptor.is_open()) { exit(1); }
 ...
 public:
  Data(float* data, int num_of_points, int dim_of_point)
    : data_(data), num_of_points_(num_of_points), dim_of_point_(dim_of_point)
  {
    if (data_ == nullptr) { throw std::runtime_error("data is nullptr"); }
  }

  Data(string path) { ReadVectorsFromFiles(path); }

  float* GetFirstPositionofPoint(int point_id) const
  {
    if (point_id >= num_of_points_) { throw std::runtime_error("point_id out of range"); }
  ...
  inline int GetNumPoints() { return num_of_points_; }

  int GetDimofPoints() { return dim_of_point_; }
};
