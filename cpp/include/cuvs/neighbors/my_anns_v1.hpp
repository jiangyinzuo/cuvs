...
/**
 * @defgroup my_anns_v1_cpp_index_params my_anns_v1 index build parameters
 * @{
 */

/**
 * @brief ANN parameters used by my_anns_v1 to build knn graph
 *
 */
...
struct ivf_pq_params {
   ...
   *   auto pq_params =
   *     my_anns_v1::graph_build_params::ivf_pq_params(dataset.extents());
...
struct index_params : cuvs::neighbors::index_params {
   ...
   *
   * @code{.cpp}
   * my_anns_v1::index_params params;
   * // 1. Choose IVF-PQ algorithm
   * params.graph_build_params = my_anns_v1::graph_build_params::ivf_pq_params(dataset.extent,
   * params.metric);
   *
   ...
   * update the dataset using my_anns_v1::update_dataset.
   *
   ...
   * the user needs to build only the search graph but does not intend to search it using my_anns_v1
   ...
   *   my_anns_v1::index_params index_params;
   *   // update index_params to only build the my_anns_v1 graph
   *   index_params.attach_dataset_on_build = false;
   *   auto index = my_anns_v1::build(res, index_params, dataset.view());
   ...
   *   my_anns_v1::search(res, search_params, index, queries, neighbors, distances);
   * @endcode
   */
...
/**
 * @defgroup my_anns_v1_cpp_search_params my_anns_v1 index search parameters
 * @{
 */
...
/**
 * @defgroup my_anns_v1_cpp_extend_params my_anns_v1 index extend parameters
 * @{
 */
...
/**
 * @defgroup my_anns_v1_cpp_merge_params my_anns_v1 index merge parameters
 * @{
 */

/**
 * @brief Determines the strategy for merging my_anns_v1 graphs.
 *
 * @note Currently, only the PHYSICAL strategy is supported.
 */
enum MergeStrategy {
  /**
   * @brief Physical merge: Builds a new my_anns_v1 graph from the union of dataset points
   * in existing my_anns_v1 graphs.
   *
   * This is expensive to build but does not impact search latency or quality.
   * Preferred for many smaller my_anns_v1 graphs.
   *
...
/**
 * @brief Parameters for merging my_anns_v1 indexes.
 */
struct merge_params {
   ...
   */
  explicit merge_params(const my_anns_v1::index_params& params) : output_index_params(params) {}

  /// Parameters for creating the output index.
  my_anns_v1::index_params output_index_params;
...
/**
 * @defgroup my_anns_v1_cpp_index my_anns_v1 index type
 * @{
 */

/**
 * @brief my_anns_v1 index.
 *
...
struct index : cuvs::neighbors::index {
  using index_params_type  = my_anns_v1::index_params;
  using search_params_type = my_anns_v1::search_params;
 ...
 public:
   ...
   * Usage examples:
   *
   * - my_anns_v1 index is normally created by the my_anns_v1::build
   * @code{.cpp}
   ...
   *   auto index = my_anns_v1::build(res, index_params, dataset);
   ...
   *   my_anns_v1::search(res, search_params, index, queries, neighbors.view(), distances.view());
   * @endcode
   ...
   *   my_anns_v1::index<T, IdxT> index(res, metric, raft::make_const_mdspan(dataset.view()),
  ...
  {
    RAFT_LOG_DEBUG("Copying my_anns_v1 knn graph from host to device");
  ...
  /** entry points [num_entry_points, dim] */
  raft::device_matrix_view<T, uint32_t, raft::row_major> entry_points(
    uint32_t num_entry_points) noexcept
  {
    auto dataset_view = dataset();
    auto view = raft::make_device_matrix_view(dataset_view.data_handle(), num_entry_points, dim());
    return view;
  }

  raft::device_matrix_view<const T, uint32_t, raft::row_major> entry_points(
  ...
  void precompute_entry_point_norms(raft::resources const& res);

  void allocate_entry_point_norms(raft::resources const& res)
  {
    ...
    switch (metric_) {
      ...
      case cuvs::distance::DistanceType::CosineExpanded:
        ...
        break;
      default: query_norms_ = std::nullopt;
    }
  }
   ...
   */
  std::optional<raft::device_vector_view<const T, uint32_t>> query_norms() const noexcept
  {
    if (query_norms_.has_value()) {
      return std::make_optional<raft::device_vector_view<const T, uint32_t>>(query_norms_->view());
    } else {
      return std::nullopt;
    }
  }
 ...
 private:
  ...
  std::optional<raft::device_vector<T, uint32_t>> query_norms_;
};
/**
 * @}
 */
...
/**
 * @defgroup my_anns_v1_cpp_index_build my_anns_v1 index build functions
 * @{
 */
 ...
 *
 * @return the constructed my_anns_v1 index
 */
...
auto build(raft::resources const& res,
  ...
  -> my_anns_v1::index<half, uint32_t>;
...
auto build(raft::resources const& res,
  ...
  -> my_anns_v1::index<int8_t, uint32_t>;
...
/**
 * @defgroup my_anns_v1_cpp_index_extend my_anns_v1 extend functions
 * @{
 */

/** @brief Add new vectors to a my_anns_v1 index
 *
 * Usage example:
 * @code{.cpp}
 ...
 *
 *   my_anns_v1::extend_params params;
 *   my_anns_v1::extend(res, params, raft::make_const_mdspan(additional_dataset.view()), index);
 * @endcode
 *
 * @param[in] handle raft resources
 * @param[in] params extend params
 * @param[in] additional_dataset additional dataset on device memory
 * @param[in,out] idx my_anns_v1 index
...
void extend(
  ...
  my_anns_v1::index<float, uint32_t>& idx,
...
void extend(
  ...
  my_anns_v1::index<int8_t, uint32_t>& idx,
...
void extend(
  ...
  my_anns_v1::index<uint8_t, uint32_t>& idx,
...
/**
 * @defgroup my_anns_v1_cpp_index_search my_anns_v1 search functions
 * @{
 * @brief Search ANN using the constructed index.
 *
 * See the [my_anns_v1::build](#my_anns_v1::build) documentation for a usage example.
 *
 * @param[in] res raft resources
 * @param[in] params configure the search
 * @param[in] idx my_anns_v1 index
...
void search(raft::resources const& res,
            ...
            const my_anns_v1::index<float, uint32_t>& index,
...
void search(raft::resources const& res,
            my_anns_v1::search_params const& params,
            const my_anns_v1::index<half, uint32_t>& index,
...
void search(raft::resources const& res,
            my_anns_v1::search_params const& params,
            const my_anns_v1::index<int8_t, uint32_t>& index,
...
void search(raft::resources const& res,
            my_anns_v1::search_params const& params,
            const my_anns_v1::index<uint8_t, uint32_t>& index,
...
/**
 * @defgroup my_anns_v1_cpp_serialize my_anns_v1 serialize functions
 * @{
 */
 ...
 * // create an index with `auto index = my_anns_v1::build(...);`
 * my_anns_v1::serialize(handle, filename, index);
 * @endcode
 *
 ...
 * my_anns_v1::index<float, uint32_t> index;
 * my_anns_v1::deserialize(handle, filename, &index);
 * @endcode
 *
 * @param[in] handle the raft handle
 * @param[in] filename the name of the file that stores the index
 * @param[out] index the my_anns_v1 index
 */
 ...
 * // create an index with `auto index = my_anns_v1::build(...);`
 * my_anns_v1::serialize(handle, os, index);
 * @endcode
 *
 * @param[in] handle the raft handle
 ...
 * my_anns_v1::index<float, uint32_t> index;
 * my_anns_v1::deserialize(handle, is, &index);
 * @endcode
 *
 * @param[in] handle the raft handle
 ...
 * my_anns_v1::index<half, uint32_t> index;
 * my_anns_v1::deserialize(handle, filename, &index);
 * @endcode
 *
 * @param[in] handle the raft handle
 ...
 * my_anns_v1::index<int8_t, uint32_t> index;
 * my_anns_v1::deserialize(handle, filename, &index);
 * @endcode
 *
 * @param[in] handle the raft handle
 ...
 * my_anns_v1::index<uint8_t, uint32_t> index;
 * my_anns_v1::deserialize(handle, filename, &index);
 * @endcode
 *
 * @param[in] handle the raft handle
...
/**
 * Write the my_anns_v1 built index as a base layer HNSW index to an output stream
 ...
 * @code{.cpp}
 * #include <raft/core/resources.hpp>
 * #include <cuvs/neighbors/my_anns_v1_serialize.hpp>
 *
 * raft::resources handle;
 *
 * // create an output stream
 * std::ostream os(std::cout.rdbuf());
 * // create an index with `auto index = raft::my_anns_v1::build(...);`
 * my_anns_v1::serialize_to_hnswlib(handle, os, index);
 * @endcode
 *
 * @param[in] handle the raft handle
 * @param[in] os output stream
...
/**
 * Save a my_anns_v1 build index in hnswlib base-layer-only serialized format
 * NOTE: The saved index can only be read by the hnswlib wrapper in cuVS,
 *       as the serialization format is not compatible with the original hnswlib.
 *
 * Experimental, both the API and the serialization format are subject to change.
 ...
 * std::string filename("/path/to/index");
 * // create an index with `auto index = raft::my_anns_v1::build(...);`
 * my_anns_v1::serialize_to_hnswlib(handle, filename, index);
 * @endcode
 *
 *
 * @param[in] handle the raft handle
...
/**
 * @defgroup my_anns_v1_cpp_index_merge my_anns_v1 index build functions
 * @{
 */

/** @brief Merge multiple my_anns_v1 indices into a single index.
 *
 * This function merges multiple my_anns_v1 indices into one, combining both the datasets and graph
 * structures.
 *
 ...
 *
 *   auto index0 = my_anns_v1::build(res, index_params, dataset0);
 *   auto index1 = my_anns_v1::build(res, index_params, dataset1);
 *
 *   std::vector<my_anns_v1::index<float, uint32_t>*> indices{&index0, &index1};
 *   my_anns_v1::merge_params params{index_params};
 *
 *   auto merged_index = my_anns_v1::merge(res, params, indices);
 * @endcode
 *
 * @param[in] res RAFT resources used for the merge operation.
 * @param[in] params Parameters that control the merging process.
 * @param[in] indices A vector of pointers to the my_anns_v1 indices to merge. All indices must:
 *                    - Have attached datasets with the same dimension.
 *
 * @return A new my_anns_v1 index containing the merged indices, graph, and dataset.
 */
auto merge(raft::resources const& res,
           const my_anns_v1::merge_params& params,
           std::vector<my_anns_v1::index<float, uint32_t>*>& indices)
  -> my_anns_v1::index<float, uint32_t>;
 ...
 *   auto index1 = my_anns_v1::build(res, index_params, dataset1);
 *
 *   std::vector<my_anns_v1::index<half, uint32_t>*> indices{&index0, &index1};
 *   my_anns_v1::merge_params params{index_params};
 *
 *   auto merged_index = my_anns_v1::merge(res, params, indices);
 * @endcode
...
auto merge(raft::resources const& res,
           const my_anns_v1::merge_params& params,
           std::vector<my_anns_v1::index<half, uint32_t>*>& indices)
  -> my_anns_v1::index<half, uint32_t>;
 ...
 *   auto index1 = my_anns_v1::build(res, index_params, dataset1);
 *
 *   std::vector<my_anns_v1::index<int8_t, uint32_t>*> indices{&index0, &index1};
 *   my_anns_v1::merge_params params{index_params};
 *
 *   auto merged_index = my_anns_v1::merge(res, params, indices);
 * @endcode
...
auto merge(raft::resources const& res,
           const my_anns_v1::merge_params& params,
           std::vector<my_anns_v1::index<int8_t, uint32_t>*>& indices)
  -> my_anns_v1::index<int8_t, uint32_t>;
 ...
 *   auto index1 = my_anns_v1::build(res, index_params, dataset1);
 *
 *   std::vector<my_anns_v1::index<uint8_t, uint32_t>*> indices{&index0, &index1};
 *   my_anns_v1::merge_params params{index_params};
 *
 *   auto merged_index = my_anns_v1::merge(res, params, indices);
 * @endcode
...
auto merge(raft::resources const& res,
           const my_anns_v1::merge_params& params,
           std::vector<my_anns_v1::index<uint8_t, uint32_t>*>& indices)
  -> my_anns_v1::index<uint8_t, uint32_t>;
/**
 * @}
 */
