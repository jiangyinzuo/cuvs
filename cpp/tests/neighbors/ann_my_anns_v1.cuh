...
struct test_my_anns_v1_sample_filter {
...
inline ::std::ostream& operator<<(::std::ostream& os, const AnnMyAnnsV1Inputs& p)
{
...
class AnnMyAnnsV1Test : public ::testing::TestWithParam<AnnMyAnnsV1Inputs> {
 public:
  AnnMyAnnsV1Test()
    : stream_(raft::resource::get_cuda_stream(handle_)),
      ps(::testing::TestWithParam<AnnMyAnnsV1Inputs>::GetParam()),
      database(0, stream_),
 ...
 protected:
  void testMyAnnsV1()
  {
    ...
    std::vector<IdxT> indices_MyAnnsV1(queries_size);
    std::vector<IdxT> indices_naive(queries_size);
    std::vector<DistanceT> distances_MyAnnsV1(queries_size);
...

    {
...

      {
        ...
        index_params.metric = ps.metric;  // Note: currently ony the my_anns_v1::index_params metric is
        ...
        switch (ps.build_algo) {
          case graph_build_algo::IVF_PQ:
            ...
            if (ps.ivf_pq_search_refine_ratio) {
              std::get<cuvs::neighbors::my_anns_v1::graph_build_params::ivf_pq_params>(
...

        {
          ...
          my_anns_v1::index<DataT, IdxT> index(handle_, index_params.metric);
          if (ps.host_dataset) {
            ...
            index = my_anns_v1::build(handle_, index_params, database_host_view);
          } else {
            index = my_anns_v1::build(handle_, index_params, database_view);
          };

          my_anns_v1::serialize(handle_, "my_anns_v1_index", index, ps.include_serialized_dataset);
        }

        my_anns_v1::index<DataT, IdxT> index(handle_);
        my_anns_v1::deserialize(handle_, "my_anns_v1_index", &index);
        ...
        raft::update_host(distances_MyAnnsV1.data(), distances_dev.data(), queries_size, stream_);
        raft::update_host(indices_MyAnnsV1.data(), indices_dev.data(), queries_size, stream_);
      ...
      //   print_vector("C", indices_MyAnnsV1.data() + i * ps.k, ps.k, std::cout);
      //   print_vector("T", distances_naive.data() + i * ps.k, ps.k, std::cout);
      //   print_vector("C", distances_MyAnnsV1.data() + i * ps.k, ps.k, std::cout);
      // }
 ...
 private:
  ...
  AnnMyAnnsV1Inputs ps;
  rmm::device_uvector<DataT> database;
  rmm::device_uvector<DataT> search_queries;
};

template <typename DistanceT, typename DataT, typename IdxT>
class AnnMyAnnsV1AddNodesTest : public ::testing::TestWithParam<AnnMyAnnsV1Inputs> {
 public:
  AnnMyAnnsV1AddNodesTest()
    : stream_(raft::resource::get_cuda_stream(handle_)),
      ps(::testing::TestWithParam<AnnMyAnnsV1Inputs>::GetParam()),
      database(0, stream_),
      search_queries(0, stream_)
...

    {
...

      {
        ...
        if (ps.host_dataset) {
          ...
          index = my_anns_v1::build(handle_, index_params, initial_database_view);
        };
        ...
        my_anns_v1::extend_params extend_params;
        my_anns_v1::extend(handle_,
                      extend_params,
...
template <typename DistanceT, typename DataT, typename IdxT>
class AnnMyAnnsV1FilterTest : public ::testing::TestWithParam<AnnMyAnnsV1Inputs> {
 public:
  AnnMyAnnsV1FilterTest()
    : stream_(raft::resource::get_cuda_stream(handle_)),
      ps(::testing::TestWithParam<AnnMyAnnsV1Inputs>::GetParam()),
      database(0, stream_),
      search_queries(0, stream_)
 ...
 protected:
  void testMyAnnsV1()
  {
...

    {
      rmm::device_uvector<DistanceT> distances_naive_dev(queries_size, stream_);
      rmm::device_uvector<IdxT> indices_naive_dev(queries_size, stream_);
      auto* database_filtered_ptr = database.data() + test_my_anns_v1_sample_filter::offset * ps.dim;
      cuvs::neighbors::naive_knn<DistanceT, DataT, IdxT>(
        ...
        ps.n_queries,
        ps.n_rows - test_my_anns_v1_sample_filter::offset,
        ps.dim,
        ps.k,
        ps.metric);
      raft::linalg::addScalar(indices_naive_dev.data(),
                              indices_naive_dev.data(),
                              IdxT(test_my_anns_v1_sample_filter::offset),
                              queries_size,
                              stream_);
      raft::update_host(distances_naive.data(), distances_naive_dev.data(), queries_size, stream_);
      raft::update_host(indices_naive.data(), indices_naive_dev.data(), queries_size, stream_);
...

    {
...

      {
        ...
        auto removed_indices =
          raft::make_device_vector<int64_t, int64_t>(handle_, test_my_anns_v1_sample_filter::offset);
        thrust::sequence(
      ...
      for (int q = 0; q < ps.n_queries; q++) {
        for (int i = 0; i < ps.k; i++) {
          const auto n      = indices_MyAnnsV1[q * ps.k + i];
          unacceptable_node = unacceptable_node | !test_my_anns_v1_sample_filter()(q, n);
        }
      }
...
template <typename DistanceT, typename DataT, typename IdxT>
class AnnMyAnnsV1IndexMergeTest : public ::testing::TestWithParam<AnnMyAnnsV1Inputs> {
 public:
  AnnMyAnnsV1IndexMergeTest()
    : stream_(raft::resource::get_cuda_stream(handle_)),
      ps(::testing::TestWithParam<AnnMyAnnsV1Inputs>::GetParam()),
      database(0, stream_),
      search_queries(0, stream_)
  ...
  void testMyAnnsV1()
  {
...

    {
...

      {
        ...
        my_anns_v1::index<DataT, IdxT> index0(handle_);
        my_anns_v1::index<DataT, IdxT> index1(handle_);
        if (ps.host_dataset) {
          {
            std::optional<raft::host_matrix<DataT, int64_t>> database_host{std::nullopt};
            ...
            index0 = my_anns_v1::build(handle_, index_params, database_host_view);
          }
          {
            std::optional<raft::host_matrix<DataT, int64_t>> database_host{std::nullopt};
            ...
            index1 = my_anns_v1::build(handle_, index_params, database_host_view);
          }
        } else {
          index0 = my_anns_v1::build(handle_, index_params, database0_view);
          index1 = my_anns_v1::build(handle_, index_params, database1_view);
        };
        std::vector<my_anns_v1::index<DataT, IdxT>*> indices{&index0, &index1};
        my_anns_v1::merge_params merge_params{index_params};
        auto index = my_anns_v1::merge(handle_, merge_params, indices);
...
inline std::vector<AnnMyAnnsV1Inputs> generate_inputs()
{
  ...
  std::vector<AnnMyAnnsV1Inputs> inputs = raft::util::itertools::product<AnnMyAnnsV1Inputs>(
    {100},
    {1000},
    {1, 8, 17},
    {16},  // k
  ...
  auto inputs2 = raft::util::itertools::product<AnnMyAnnsV1Inputs>(
    {1, 100},
    {1000},
    {8},
    {1, 16},  // k
  ...
  inputs2 = raft::util::itertools::product<AnnMyAnnsV1Inputs>(
    {2},
    {3, 5, 31, 32, 64, 101},
    {1, 10},
    {2},  // k
  ...
  inputs2 =
    raft::util::itertools::product<AnnMyAnnsV1Inputs>({100},
                                                   {1000},
...
inline std::vector<AnnMyAnnsV1Inputs> generate_addnode_inputs()
{
  // changing dim
  std::vector<AnnMyAnnsV1Inputs> inputs =
    raft::util::itertools::product<AnnMyAnnsV1Inputs>({100},
                                                   {1000},
...
inline std::vector<AnnMyAnnsV1Inputs> generate_filtering_inputs()
{
...
const std::vector<AnnMyAnnsV1Inputs> inputs           = generate_inputs();
const std::vector<AnnMyAnnsV1Inputs> inputs_addnode   = generate_addnode_inputs();
const std::vector<AnnMyAnnsV1Inputs> inputs_filtering = generate_filtering_inputs();
